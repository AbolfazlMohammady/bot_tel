...
# 📅 تابع اضافه کردن کوین به همه کاربران
def add_coins_to_all_users():
    conn = sqlite3.connect('db.WolfBotDatabase')
    cursor = conn.cursor()
    cursor.execute("UPDATE users SET Coin = Coin + 1000")
...
# ✅ ترد برنامه‌ریزی جدا از بقیه
threading.Thread(target=run_schedule, daemon=True).start()

# 📨 هندلر استارت
@bot.message_handler(commands=['start'])
def account(message):
    print(f"/start دریافت شد از {message.from_user.id}")
    ...
    try:
        ...
        bot.send_message(message.chat.id, "عکسی برای ارسال پیدا نشد.")
